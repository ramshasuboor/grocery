...
export const addInvoice = async (req, res) => {
  console.log(req.body, "customerId")
  try {
    const { customer, customerName, rows, totals, paidAmount, invoiceNo, date } = req.body;

    // 🔹 Pichla invoice find karo taaki uska closing balance mile
    const lastInvoice = await Invoice.findOne({ customer }).sort({ createdAt: -1 });

    const openingBalance = lastInvoice ? lastInvoice.closingBalance : 0;

    // Total amount nikalo (agar frontend se totals aa rahe hain)
    const grossTotal = totals?.totalAmount || 0;

    // 🔹 Closing balance formula: opening + grossTotal - paid
    const closingBalance = openingBalance + grossTotal - (paidAmount || 0);
    ...
    const invoice = new Invoice({
      ...
      totals,
      paidAmount,
      balanceAmount: grossTotal - (paidAmount || 0), // ✅ Net balance for this invoice
      openingBalance,   // ✅ Backend se assign
      closingBalance,   // ✅ Backend se assign
      invoiceNo: invoiceNo || ("INV-" + new Date().getTime()),
...
export const getInvoice = async (req, res) => {
  ...
  try {
    ...
    //   (openingBalance + (invoice.totals?.totalAmount || 0) - (invoice.paidAmount || 0));
...
//     const openingBalance = invoice.openingBalance ?? 0;
//     const closingBalance = invoice.closingBalance ?? (openingBalance + invoice.totals.totalAmount - invoice.paidAmount);
